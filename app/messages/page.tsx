...
import { getSharedFiles } from "../utils/fileStorage";
...
export default function MessagesPage() {
  ...
  const handleImageUpload = async (file: File) => {
    ...
    reader.onload = async (event) => {
      ...
      if (typeof window !== "undefined") {
        const { addSharedFile } = await import("../utils/fileStorage");
        addSharedFile({
          clientId: selectedClientId,
          fileName: file.name,
        ...
        <div className="flex-1 flex flex-col">
          {selectedClientId ? (
            <>
              ...
              {/* Messages */}
              <div className="flex-1 overflow-y-auto px-6 py-8 bg-gradient-to-b from-gray-900/50 to-gray-800/50">
                        ...
                        >
                          <div
                            className={`max-w-[75%] rounded-3xl px-5 py-3 shadow-2xl border transition-all hover:scale-[1.02] ${
                              msg.sender === "coach"
                                ? "bg-gradient-to-br from-gray-700 to-gray-900 text-white border-white/20 rounded-br-md"
                                : "bg-gradient-to-br from-red-600 to-red-700 text-white border-red-500/30 rounded-bl-md"
                            }`}
                          >
                            {isImage && imageUrl ? (
                              <div className="mb-2">
